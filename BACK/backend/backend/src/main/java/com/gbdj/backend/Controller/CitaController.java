...
public class CitaController {
    ...
    public ResponseEntity<?> solicitarCita(@RequestBody SolicitarCitaRequest request) {
        ...
        // 👇 Datos de traspaso (pueden ser null si no es traspaso)
        citaData.put("P_ES_DUENIO_REGISTRADO", request.getEsDuenioRegistrado());
        citaData.put("P_CEDULA_DUENIO_ACTUAL", request.getCedulaDuenioActual());
        citaData.put("P_NOMBRE_DUENIO_ACTUAL", request.getNombreDuenioActual());
        citaData.put("P_APELLIDO_DUENIO_ACTUAL", request.getApellidoDuenioActual());

        log.info("Enviando a APEX: {}", citaData);
        ...
        if (response != null && "OK".equals(response.get("status"))) {
            ...
            String mensaje = response != null ? response.get("mensaje").toString() : "Error al solicitar cita";
    ...
    public ResponseEntity<?> listarCitasPendientes(@PathVariable Long cedulaAsesor) {
        ...
        if (response != null && "OK".equals(response.get("status"))) {
            ...
            String mensaje = response != null ? response.get("mensaje").toString() : "Error al listar citas";
            return ResponseEntity.status(500).body(Map.of(
                    "status", "ERROR",
                    "mensaje", mensaje));
        }
    ...
    public ResponseEntity<?> atenderCita(@RequestBody AtenderCitaRequest request) {
        ...
        if (response != null && "OK".equals(response.get("status"))) {
            ...
            String mensaje = response != null ? response.get("mensaje").toString() : "Error al atender cita";
            return ResponseEntity.status(500).body(Map.of(
                    "status", "ERROR",
                    "mensaje", mensaje));
        }
    ...
    public ResponseEntity<?> listarCitasAgendadas(@PathVariable Long cedulaAsesor) {
        Map<String, Object> response = citaService.listarCitasAgendadas(cedulaAsesor);
    ...
    public ResponseEntity<?> completarCita(@RequestBody CompletarCitaRequest request) {
        ...
        if (response != null && "OK".equals(response.get("status"))) {
            ...
            String mensaje = response != null ? response.get("mensaje").toString() : "Error al completar cita";
            return ResponseEntity.status(500).body(Map.of(
                    "status", "ERROR",
                    "mensaje", mensaje));
        }
